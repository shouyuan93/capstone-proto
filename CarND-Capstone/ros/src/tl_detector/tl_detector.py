...
class TLDetector(object):
	...
	for i in range(len(waypoints_list)):
		new_dist = self.calc_distance_points_3D(pose.position,waypoints_list[i].pose.pose.position)
		if dist > new_dist:
			dist = new_dist
			waypoint = i
        return waypoint
	...
	if hasattr(self, 'waypoints') and hasattr(self, 'pose'):
		...
		for light_pos in light_positions:
			light_wp, tl_candid,tl_dist = self.calculate_distance_to_traffic_light(car_wp,light_pos)
			if (tl_dist < min_dist) and((car_wp %len(self.waypoints.waypoints))<(light_wp%len(self.waypoints.waypoints))) and (tl_dist <OBSERV_DIST):
    ...
    """
    def calculate_distance_to_traffic_light(self,car_waypoint,light_position):
	...
	tl_dist = self.calc_distance_coords_2D(self.waypoints.waypoints[car_waypoint].pose.pose.position.x,self.waypoints.waypoints[car_waypoint].pose.pose.position.y,self.waypoints.waypoints[light_waypoint].pose.pose.position.x,self.waypoints.waypoints[light_waypoint].pose.pose.position.y)
